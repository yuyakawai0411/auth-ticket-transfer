...
RSpec.describe "Tickets", type: :request do
  ...
  describe 'GET #create' do
    ...
    let(:ticket) { FactoryBot.create(:ticket, ticket_name: 'ジャンプフェスタ', event_date: '2022-08-22', category_id: 1, event_id: event.id, user_id: user.id) }
    # let(:ticket_not_exist_user) { ticket_name: 'ジャンプフェスタ', event_date: '2022-08-22', category_id: 1, event_id: event.id, user_id: user_not_exist.id }
      context '存在するユーザーにチケットを発券した時' do
        it 'Ticketモデルのカウントが+1される' do
          expect{
            post "/events/#{event.id}/tickets", params:  { ticket_name: 'ジャンプフェスタ', event_date: '2022-08-22', category_id: 1, event_id: event.id, user_id: user.id }
      ...
      context '存在しないユーザーにチケットを発券した時' do
        it 'エラーメッセージが返される' do
          post "/events/#{event.id}/tickets", params:  { ticket_name: 'ジャンプフェスタ', event_date: '2022-08-22', category_id: 1, event_id: event.id, user_id: user_not_exist.id }
          json = JSON.parse(response.body)
