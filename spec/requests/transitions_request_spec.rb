...
RSpec.describe "Transitions", type: :request do
  ...
  let!(:transition) { FactoryBot.create(:transition, ticket_id: ticket.id, sender_id: user_sender.id, receiver_id: user_receiver.id) }
    context 'チケットの譲渡にて、paramsを正しくリクエストした時' do
      subject { post "/v1/users/#{user_sender.id}/tickets/#{ticket.id}/transitions", params: { receiver_id: user_receiver.id }  }
      ...
      it 'ticketsテーブルでticketのuser_idがreceiverのidになる' do
        get "/v1/users/#{user_sender.id}/tickets/#{ticket.id}"
        json = JSON.parse(response.body)
        expect(json['data']['user_id']).to eq(user_sender.nickname)
        subject
        get "/v1/users/#{user_receiver.id}/tickets/#{ticket.id}"
        json = JSON.parse(response.body)
    ...
    context 'チケットの譲渡にて、paramsを正しくリクエストしなかった時' do
      subject { post "/v1/users/#{user_sender.id}/tickets/#{ticket.id}/transitions", params: { receiver_id: user_not_exist }  }
    ...
    context '所持していないチケットを譲渡しようとした時' do
      subject { post "/v1/users/#{user_sender.id}/tickets/#{ticket_not_exist}/transitions", params:  { receiver_id: user_receiver.id } }
      it 'エラーメッセージが返される' do
        subject
        json = JSON.parse(response.body)
  ...
  let!(:transition) { FactoryBot.create(:transition) }
    context 'user_receiverが所持するチケットを検索した時' do
      subject { get "/v1/users/#{transition.ticket.user.id}/tickets/#{transition.ticket.id}/transitions"  }
    ...
    context 'user_receiverが所持しないチケットを検索した時' do
      subject { get "/v1/users/#{user_receiver.id}/tickets/#{ticket_not_exist}/transitions" }
      it 'エラーメッセージが返される' do 
        subject
        json = JSON.parse(response.body)
  ...
  let(:transition_not_exist) { transition.id + 1 }
    context '存在する譲渡履歴を検索した時' do
      subject { get "/v1/users/#{transition.ticket.user.id}/tickets/#{transition.ticket.id}/transitions/#{transition.id}" }
      it 'transitionに正しい値がある' do
        subject
        json = JSON.parse(response.body)
    ...
    context '存在しない譲渡履歴を検索した時' do
      subject { get "/v1/users/#{transition.ticket.user.id}/tickets/#{transition.ticket.id}/transitions/#{transition_not_exist}" }
    ...
    context 'user_receiverが所持しないチケットを検索した時' do
      subject { get "/v1/users/#{user_receiver.id}/tickets/#{ticket_not_exist}/transitions/#{transition.id}" }
      it 'エラーメッセージが返される' do 
        subject
        json = JSON.parse(response.body)
        expect(json['message']).to eq('所持していないチケットです') 
